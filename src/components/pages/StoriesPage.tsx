...
const storiesTypes = [
  { key: 'latest-stories', title: 'Latest stories', category: null },
  { key: 'stories', title: 'Stories', category: 'Stories' }
] as const;

export function StoriesPage({ activeCategory }: storiesPageProps) {
  ...
  async function fetchArticles() {
    try {
      ...
      const storiesArticles = (data || []).filter(article => 
        article.categories?.title?.includes('Stories')
      );

      setArticles(storiesArticles as ArticleWithCategory[]);
  ...
  const getArticlesForType = (storiesType: typeof storiesTypes[number]) => {
    if (storiesType.key === 'latest-stories') {
      return articles.slice(0, 3);
    }
    
    return articles.filter(article => 
      article.categories?.title === storiesType.category
    );
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {storiesTypes.map((storiesType) => {
        const typeArticles = getArticlesForType(storiesType);
        
        if (typeArticles.length === 0) return null;

        return (
          <section key={storiesType.key} className="space-y-4">
            <div className="border-b border-gray-200 pb-2">
              <h2 className="text-2xl font-bold text-gray-900">{storiesType.title}</h2>
            </div>
      ...
      {articles.length === 0 && (
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No stories articles found</h3>
            <p className="text-gray-600">
              {searchQuery 
                ? `No stories articles match your search for "${searchQuery}"`
                : 'No stories articles have been published yet'
              }
            </p>
          </CardContent>
        </Card>
