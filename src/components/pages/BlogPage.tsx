...
const blogTypes = [
  { key: 'latest-blogs', title: 'Latest Blogs', category: null },
  { key: 'blogs', title: 'Blogs', category: 'Blogs' }
] as const;

export function BlogPage({ activeCategory }: BlogPageProps) {
  ...
  async function fetchArticles() {
    try {
      ...
      const blogArticles = (data || []).filter(article => 
        article.categories?.title?.includes('Blogs')
      );

      setArticles(blogArticles as ArticleWithCategory[]);
  ...
  const getArticlesForType = (blogType: typeof blogTypes[number]) => {
    if (blogType.key === 'latest-blogs') {
      return articles.slice(0, 3);
    }
    
    return articles.filter(article => 
      article.categories?.title === blogType.category
    );
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {blogTypes.map((blogType) => {
        const typeArticles = getArticlesForType(blogType);
        
        if (typeArticles.length === 0) return null;

        return (
          <section key={blogType.key} className="space-y-4">
            <div className="border-b border-gray-200 pb-2">
              <h2 className="text-2xl font-bold text-gray-900">{blogType.title}</h2>
            </div>
